...
public class G_Maps extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final DatabaseHandlerActivity db = new DatabaseHandlerActivity(getApplicationContext());
		location_list = db.getAllLocations();
		db.close();
		
		if(location_list.isEmpty())
		{
//			Toast.makeText(getApplicationContext(), "Pre-Location", Toast.LENGTH_SHORT).show();
...
//	        String formattedDate = df.format(c.getTime());
	        ...
	        .position(CIU).title("Location").snippet("Latitude=10.52306750, Longitude=76.22221059999992"));
			...
			for(Locations_viewer locations_viewer:location_list)
			{
				...
				CIU = new LatLng(latitude, longitude);
				 mMap.addMarker(new MarkerOptions()
	             .position(CIU).title("Location").snippet("Latitude="+locations_viewer.getLat()+",  Longitude="+locations_viewer.getLong()));
			}
		}
...
//		else
//		{
//			CIU = new LatLng(10.000886500000000000, 76.299572799999960000);
//		}
